...
$ApiDir       = Join-Path $PlatformRoot ".." "api"
$UiDir        = Join-Path $PlatformRoot ".." "ui"
...
$ApiAvailable = Test-Path $ApiDir
$UiAvailable  = Test-Path $UiDir

if ($ApiAvailable) { Write-OK "api/ found" }
else { Write-Warn "api/ not found at $ApiDir — run this from a complete FleetBits checkout" }

if ($UiAvailable) { Write-OK "ui/ found" }
else { Write-Warn "ui/ not found at $UiDir — run this from a complete FleetBits checkout" }
...
"@ | Set-Content -Encoding utf8 $apiEnv
        ...
        Write-Warn "api/.env already exists — not changed (pass -Force to overwrite)"
    }
}
...
"@ | Set-Content -Encoding utf8 $uiEnv
        Write-OK "ui/.env written"
    } else {
        Write-Warn "ui/.env already exists — not changed (pass -Force to overwrite)"
    }
}
