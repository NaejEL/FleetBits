...
### Producteur « voie serveur » — `FleetBits-api`
...
- `devices.py:144-145` fabrique `https://prometheus.{FLEET_DOMAIN}` et `https://loki.{FLEET_DOMAIN}`. `FLEET_DOMAIN` n'existe pas dans `app/config.py` ; les hôtes réellement exposés par `FleetBits-platform/docker/caddy/Caddyfile:46` et `:57` sont `metrics.{FLEET_DOMAIN}` avec chemin `/api/v1/write` et `logs.{FLEET_DOMAIN}` avec chemin `/loki/api/v1/push`. Ni l'hôte ni le chemin ne correspondent.

### Producteur « voie conteneur » et les quatre consommateurs — `FleetBits-agent`
...
### Producteur « voie automatisation » — `FleetBits-platform`
...
- Une **migration vers un monorepo est prévue une fois les findings de l'audit soldés**. Les choix de ce cycle doivent survivre à cette fusion sans redécoupage : la définition unique vit dans `FleetBits-api` (elle deviendra un module partagé), et le harnais de test reste par dépôt (chaque dépôt deviendra un package avec sa propre suite).
...
2. **Source de vérité du contrat** : `FleetBits-api`. `FleetBits-agent` et `FleetBits-platform` la dupliquent, la cohérence étant garantie par un test de comparaison d'ensembles de clés.
...
4. **Harnais de test** : `bats` (+ `shellcheck`) pour `FleetBits-agent` ; vérification du rendu de gabarit pour `FleetBits-platform`. Un point d'entrée par dépôt.
...
- `FLEET_DOMAIN` est déclaré dans `app/config.py` ; les adresses de télémétrie renvoyées désignent des hôtes et chemins effectivement exposés par le routage de `FleetBits-platform`.
...
1. Le chemin d'URL construit dans `FleetBits-agent/usr/lib/fleet-agent/firstboot.sh` pour l'appel de provisionnement contient le segment d'identifiant d'appareil et correspond, segment par segment, au chemin déclaré par le décorateur de route de `FleetBits-api/app/routers/devices.py` ; une recherche textuelle dans les trois dépôts ne fait apparaître aucune occurrence de `/devices/provision` sans identifiant. *(FleetBits-agent, FleetBits-api)*
2. Aucun fichier de `FleetBits-agent/usr/lib/fleet-agent/` ni `FleetBits-agent/container-entrypoint.sh` ne contient d'instruction `source` ou `.` appliquée au fichier d'identité ; vérifiable par recherche textuelle renvoyant zéro occurrence. *(FleetBits-agent)*
3. La lecture du fichier d'identité rejette, avec un code de sortie non nul et un message sur la sortie d'erreur, tout fichier contenant une clé absente de la liste blanche du contrat et tout fichier dont une valeur contient un caractère hors du jeu autorisé ; démontré par un test `bats` exécutant chacun des trois consommateurs sur des fixtures malveillantes. *(FleetBits-agent)*
4. Un test `bats` exécute chacun des trois consommateurs sur un fichier d'identité contenant une valeur conçue pour s'exécuter si le fichier était évalué par l'interpréteur (substitution de commande, point-virgule suivi d'une commande, expansion de variable) ; le test échoue si la commande injectée s'exécute, et vérifie l'absence de son effet observable. *(FleetBits-agent)*
5. La liste des clés produites par la route de provisionnement de `FleetBits-api`, celle produite par `FleetBits-platform/ansible/roles/fleet_agent/templates/device-identity.conf.j2`, celle produite par `FleetBits-agent/container-entrypoint.sh` et celle acceptée par la liste blanche de l'analyseur sont identiques ensemble à ensemble ; un test automatisé compare ces quatre ensembles et échoue sur toute différence. *(FleetBits-api, FleetBits-platform, FleetBits-agent)*
6. Le fichier produit par la route de provisionnement contient une clé `FLEET_API_URL` non vide ; `heartbeat.sh` exécuté sur ce fichier atteint l'émission de la requête HTTP sans déclencher son contrôle de variables obligatoires. *(FleetBits-api, FleetBits-agent)*
7. Le fichier produit par le gabarit Ansible contient les clés que `firstboot.sh` traite comme fatales lorsqu'elles manquent, ou bien `firstboot.sh` ne les traite plus comme fatales ; aucun des deux fichiers ne référence une clé que l'autre ne peut fournir. *(FleetBits-platform, FleetBits-agent)*
8. Les valeurs de `FLEET_METRICS_URL` et `FLEET_LOGS_URL` renvoyées par la route de provisionnement correspondent à un couple hôte + chemin défini dans `FleetBits-platform/docker/caddy/Caddyfile` ; un test compare les valeurs renvoyées aux motifs d'hôte et de chemin déclarés dans le `Caddyfile`. *(FleetBits-api, FleetBits-platform)*
9. `FLEET_DOMAIN` est déclaré dans `FleetBits-api/app/config.py` et injecté par `FleetBits-platform/docker/docker-compose.yml` ; démarrer l'API sans ce réglage produit une erreur de configuration explicite au démarrage, et non une erreur à l'exécution de la route. *(FleetBits-api, FleetBits-platform)*
10. `FLEET_LOGS_URL` renvoyé par la route de provisionnement satisfait l'expression régulière d'URL absolue appliquée par `generate-config.sh:107` ; vérifié par un test qui passe la valeur renvoyée à la fonction de rendu Vector. *(FleetBits-api, FleetBits-agent)*
11. `ENVIRONMENT` et `RING` figurent dans le contrat, sont renvoyés par la route de provisionnement avec des valeurs non vides, et apparaissent comme labels dans la configuration de télémétrie rendue par `generate-config.sh` ; un test inspecte la sortie rendue et échoue si l'un des deux labels manque. *(FleetBits-api, FleetBits-agent)*
12. `HEADSCALE_PREAUTH_KEY` est déclaré dans le contrat, accepté vide par l'analyseur, et son absence de valeur ne fait échouer aucun des trois consommateurs ; un test exécute `firstboot.sh` sur un fichier où la clé est présente et vide, et vérifie que la branche Headscale est ignorée sans erreur. *(FleetBits-agent, FleetBits-api)*
13. Un test de bout en bout, dans `FleetBits-api/tests/`, part d'un appareil et d'un jeton de provisionnement créés par fixture, appelle la route de provisionnement telle que `firstboot.sh` l'appelle, et vérifie que le corps de réponse est accepté sans erreur par l'analyseur de fichier d'identité de `FleetBits-agent` ; ce test échoue si le chemin, le format ou la liste de champs diverge. *(FleetBits-api)*
14. Le dépôt `FleetBits-agent` contient un point d'entrée de test `bats` exécutable par une commande unique documentée, exécuté par un workflow d'intégration continue de ce dépôt, couvrant au minimum les critères 3, 4, 11 et 12, et accompagné d'une passe `shellcheck` sur tous les scripts du dépôt. *(FleetBits-agent)*
15. Le dépôt `FleetBits-platform` contient un point d'entrée de test exécutable par une commande unique documentée, exécuté par un workflow d'intégration continue de ce dépôt, vérifiant que le rendu de `device-identity.conf.j2` avec un jeu de variables d'inventaire d'exemple est accepté par l'analyseur de `FleetBits-agent`. *(FleetBits-platform)*
16. Le mode de fichier appliqué au fichier d'identité est identique dans `FleetBits-platform/ansible/roles/fleet_agent/tasks/main.yml`, `FleetBits-agent/usr/lib/fleet-agent/firstboot.sh` et `FleetBits-agent/scripts/postinst.sh` ; vérifiable par inspection des trois fichiers. *(FleetBits-platform, FleetBits-agent)*
17. La tâche « Collect device identity (redact token) » de `FleetBits-platform/ansible/playbooks/collect_diagnostics.yml` ne laisse apparaître aucune des clés du contrat désignées comme secrètes dans la définition unique ; vérifié par un test appliquant la commande de redaction à un fichier d'identité complet et cherchant chaque valeur secrète dans la sortie. *(FleetBits-platform)*
18. `FleetBits-agent/etc/fleet/device-identity.conf.example` contient exactement les clés du contrat, sans clé supplémentaire ni manquante ; comparaison automatisée avec la liste blanche. *(FleetBits-agent)*
19. Le bloc de référence de `FleetBits-agent/README.md` décrivant le fichier d'identité liste les mêmes clés que le fichier d'exemple ; comparaison mécanique des deux listes. *(FleetBits-agent)*
20. Le point d'entrée `container-entrypoint.sh` produit, à partir des variables déclarées pour le service `vps-device` dans `FleetBits-platform/docker/docker-compose.yml`, un fichier d'identité accepté par l'analyseur ; vérifié par un test exécutant le point d'entrée hors conteneur sur ces variables. *(FleetBits-platform, FleetBits-agent)*
21. Aucun drapeau de configuration, aucune variable d'environnement ni aucun paramètre de requête ne permet de sélectionner l'ancien format ou l'ancien chemin ; l'ancienne route, l'ancien format et toute forme de lecture par `source` ne subsistent nulle part dans les quatre dépôts, vérifiable par recherche textuelle. *(FleetBits-api, FleetBits-agent, FleetBits-platform)*
22. `pre-commit run --all-files` passe sur les trois dépôts modifiés, y compris `gitleaks`, `actionlint`, et pour `FleetBits-api` `bandit -ll` et `ruff --select S`. *(FleetBits-api, FleetBits-agent, FleetBits-platform)*
23. Les suites de tests des trois dépôts touchés passent : `./.venv/bin/python -m pytest` pour `FleetBits-api`, `shellcheck` + `bats` pour `FleetBits-agent`, `docker compose config -q` + `ansible-playbook --syntax-check` + `ansible-lint` + le point d'entrée de test pour `FleetBits-platform`. *(FleetBits-api, FleetBits-agent, FleetBits-platform)*
...
- **Signalés en chemin, non traités** : l'en-tête de `firstboot.sh:18` renvoie vers un parcours « Fleet UI → Device View → [Replace device] » produisant `fleet-provision.json`, absent de `FleetBits-ui`.
